...
public class Sample {
	//call by value: 값에 의하 매개변수 전달
	//배개변수의 자료형이 value 타입
	//매개변수로 대입된 원본데이터를 메소드 안에서 변경할 수 없음
	...
	//이런 종류의 메소드를 호출할 때는 도큐먼드를 확인해 보거나
	...
	//return을 이용합니다.
	//메소드 이름앞에 void대신에 return 하고자 하는 데이터의
	//자료형을 기재하고 메소드 내에서 return데이터; 의 형식을
	//만들어주면 됩니다.
	//메소드에서 void(리턴이 없으면)type이면 메소드를 호출하고
	//수행한 후 흐름이 끊어지게 됩니다.
	//데이터를 return하면 그 return된 데이터를 가지고 다른 작업을
	//연속해서 수행 할 수 있습니다.
	//메소그 호출구문 안에서 메소드를 호출하는 chaining 형식도
	//가능합니다.
